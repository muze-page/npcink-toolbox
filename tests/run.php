...
toolbox_assert( false !== $editor_support_json && null !== json_decode( $editor_support_json, true ) && false !== strpos( $editor_support_json, '"Publish preflight": ["发布预检"]' ) && false !== strpos( $editor_support_json, '"Writing preparation": ["写作准备"]' ) && false !== strpos( $editor_support_json, '"Summary suggestions": ["摘要建议"]' ) && false !== strpos( $editor_support_json, '"Tag suggestions": ["标签建议"]' ), 'Bundled zh_CN editor script translation JSON is valid and covers fixed-flow labels.' );
...
toolbox_assert( false !== strpos( $rest, 'editor_fast_summary_suggestions' ) && false !== strpos( $rest, 'editor_fast_category_suggestions' ) && false !== strpos( $rest, 'editor_fast_tag_suggestions' ) && false !== strpos( $rest, 'empty_summary_layer_candidates' ), 'REST editor metadata shortcuts split summary, category, and tag suggestions into fast paths.' );
