...
toolbox_assert( false !== strpos( $admin_page, 'Free GPT-5.5' ) && false !== strpos( $admin_page, 'free-gpt55-article-optimization' ) && false !== strpos( $admin_page, 'free-gpt55-site-checkup' ) && false !== strpos( $admin_page, 'free-gpt55-image-alt' ) && false !== strpos( $admin_page, 'free-gpt55-smart-recommendations' ), 'Tool actions expose the free GPT-5.5 entry group.' );
toolbox_assert( false !== strpos( $admin_page, "'intent'      => 'article_optimization'" ) && false !== strpos( $admin_page, "'intent'      => 'site_checkup'" ) && false !== strpos( $admin_page, "'intent'      => 'media_alt'" ) && false !== strpos( $admin_page, "'intent'      => 'smart_recommendations'" ), 'Free GPT-5.5 tools map to the article, site, media, and recommendation intents.' );
toolbox_assert( false !== strpos( $admin_page, 'Automatic public snapshot' ) && false !== strpos( $admin_page, 'bounded sample of public site metadata or media-library metadata' ), 'Site-level GPT-5.5 tools explain bounded automatic public snapshots.' );
...
toolbox_assert( false !== strpos( $admin_js, 'Article optimization suggestions' ) && false !== strpos( $admin_js, 'Site checkup suggestions' ) && false !== strpos( $admin_js, 'Media alt suggestions' ) && false !== strpos( $admin_js, 'Next action recommendation' ), 'Admin JavaScript labels the four free GPT-5.5 result modes.' );
...
toolbox_assert( false !== strpos( $editor_js, 'PluginDocumentSettingPanel' ) && false !== strpos( $editor_js, 'Npcink Content Support' ), 'Editor JavaScript registers a Npcink Content Support document panel.' );
...
toolbox_assert( false !== strpos( $client, 'execute_image_source_cloud_request' ) && false !== strpos( $client, 'npcink_toolbox_image_source_cloud_request' ), 'Image candidates use a Cloud-managed image-source runtime seam.' );
...
toolbox_assert( false !== strpos( $client, 'npcink_toolbox_site_knowledge_cloud_request' ) && false !== strpos( $client, 'magick_ai_cloud_addon_runtime_client' ), 'Site knowledge execution uses a host filter or Cloud Addon runtime seam.' );
...
toolbox_assert( false !== strpos( $client, 'run_free_gpt55_content_support' ) && false !== strpos( $client, "'profile_id'          => 'text.free-gpt55'" ) && false !== strpos( $client, "'ability_name'        => 'npcink-toolbox/free-gpt55-content-support'" ), 'Provider client runs free GPT-5.5 content support through the dedicated hosted profile.' );
toolbox_assert( false !== strpos( $client, 'collect_free_gpt55_site_snapshot' ) && false !== strpos( $client, 'collect_free_gpt55_media_alt_snapshot' ) && false !== strpos( $client, 'smart_recommendations' ), 'Provider client gathers bounded public snapshots for GPT-5.5 site, media, and recommendation tools.' );
