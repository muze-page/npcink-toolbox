...
before Core handoff. A future "fill missing ALT only" auto-approval path must
be configured and enforced by Core policy, not by Toolbox direct apply.
...
lets selected media-library images enter a bounded batch workbench. Operators
can request a bounded one-run
